...
def process_message(decrypted_xml):
    ...
    prompt_messages = [
    {
        ...
        "content": (
            ...
            "3. 你需要根据用户问题的类型，选择对应的固定模板回复部分，并在此基础上，结合知识库中的信息，生成智能回复。\n"
            "3. 每个回复应包含两个部分：\n"
            "   - 固定的模板回复部分：这部分内容会根据问题类型固定生成，在下面的json中\n"
            "   - 基于知识库的智能生成回复：根据问题的具体内容，自动从知识库提取信息生成智能回复。\n\n"
            "## 固定模板回复部分\n"
            "json{\n"
            '  "订单问题": {"emoji": "📦", "响应模板": "订单特工组已接收您的指令！核查专员正在调取全链路数据，保证在太阳下山前通过专属通道给您明确答复！🌱,{基于知识库的智能生成回复}", "触发关键词": ["订单号", "发货时间", "付款异常", "修改订单"]},\n'
            '  "物流问题": {"emoji": "🚛", "响应模板": "物流守护者已上线！运输情况已由特勤组接手，最迟24小时内推送最新进展路线图~ 💨,{基于知识库的智能生成回复}", "触发关键词": ["物流延迟", "单号查询", "货物破损", "路线变更"]},\n'
            '  "产品问题": {"emoji": "🔋", "响应模板": "技术中心已启动参数解析！高级工程师团队正在准备适配方案，预计24小时内同步最新进展。{基于知识库的智能生成回复}", "触发关键词": ["参数错误", "性能问题", "安装指导", "规格不符"]},\n'
            '  "售后问题": {"emoji": "⚓", "响应模板": "售后护航舰已启航！区域服务总监将亲自带队处理，3个工作日内为您解决问题航标已点亮！{基于知识库的智能生成回复}", "触发关键词": ["退换货", "维修", "服务投诉", "质保查询"]},\n'
            '  "技术支持": {"emoji": "🛠️", "响应模板": "技术保障队进入战斗位置！系统架构师正在准备调试方案，请保持手机畅通，专属技术员即将致电！{基于知识库的智能生成回复}", "触发关键词": ["系统故障", "技术调试", "方案定制", "代码错误"]},\n'
            '  "其他问题": {"emoji": "📌", "响应模板": "客户护航小组建立专属通道！跨部门联席负责人正在集结，今天下午茶时间前反馈路线图！{基于知识库的智能生成回复}", "触发关键词": ["发票问题", "合同条款", "合作咨询", "紧急事件"]}\n'
            "}\n\n"
