...
def process_message(decrypted_xml):
    ...
    prompt_messages = [
    {
        ...
        "content": (
            ...
            "1. 接收到用户输入文本后，根据内容判断用户的问题类型。\n"
            "2. 每个回复应包含两个部分：\n"
            "   - 固定的模板回复部分：这部分内容会根据问题类型固定生成。\n"
            "   - 基于知识库的智能生成回复：根据问题的具体内容，自动从知识库提取信息生成智能回复。\n\n"
            "## 固定模板回复部分\n"
            "json{\n"
            '  "标准回复": {"emoji": "📋", "响应模板": "感谢您的来信，我们的专员已收到您的请求并将尽快处理。请您耐心等待，若有任何问题，欢迎随时联系。"}\n'
            "}\n\n"
            ...
            "当用户输入无法匹配的内容时，AI 会自动根据知识库内容生成相关问题的智能回复，并结合标准化模板进行回复。智能回复将根据用户输入的具体问题生成，例如："
